...
- `wear/`: standalone Wear OS application
- `tv/`: Android TV application
- `composition/`: shared product assembly compiled separately into each shell
...
The product build includes only `app/`, `wear/`, and `tv/`. `components/` is an independent Gradle build
...
components. `ANDROID_HOME` must identify that SDK. The native SDK contains armeabi-v7a, arm64-v8a
...
./gradlew :app:dependencies :wear:dependencies :tv:dependencies --write-locks
make verify
```
...
## Phone, watch and TV shells

All three applications are generated by default and consume the same locked binary SDK set.
`homestate` owns UI-independent state and the public HomeFactory; `home`, `wearhome`, and
`tvhome` provide separate mobile Material, Wear Compose, and TV Compose interfaces.
The source-only `composition` folder belongs to the product and is compiled into each shell.
It does not cause SDK source fallback. Each application owns its own session; state is not
synchronized across devices.

| Platform | Module | Release application ID | Minimum API |
| --- | --- | --- | --- |
| mobile (default) | app | `{{APPLICATION_ID}}` | configured `{{MIN_SDK}}` |
| wear | wear | `{{APPLICATION_ID}}.wear` | max(26, configured minimum) |
| tv | tv | `{{APPLICATION_ID}}.tv` | max(23, configured minimum) |

```sh
make components-bootstrap
make build PLATFORM=wear
make build PLATFORM=tv
make verify-all
ANDROID_SERIAL=<watch-serial> make test-ui PLATFORM=wear
ANDROID_SERIAL=<tv-serial> make test-ui PLATFORM=tv
make bundle-release PLATFORM=tv
make artifact-info PLATFORM=tv
PLATFORM=wear fastlane archive
```

`build`, `test`, `test-ui`, `lint`, `install-debug`, signing, artifact verification and
Fastlane releases select one shell. `build-all`, `test-all`, `lint-all`, `verify-all`
cover all three. Device tests must target an appropriate system image, using
`ANDROID_SERIAL` when multiple emulators are running. Debug identity adds the worktree
suffix after the device suffix. Wear and TV store metadata directories are
`fastlane/metadata/wear` and `fastlane/metadata/tv`; mobile retains `fastlane/metadata/android`.

Wear uses a round-screen scrolling list, rotary input and swipe-to-dismiss on the about page.
It is a standalone app with a distinct package, not a phone companion with Data Layer sync.
TV uses Leanback discovery, a banner, large controls and explicit D-pad focus navigation.
Replace the starter TV banner/icon with product artwork before distribution.
The shared native AAR contains armeabi-v7a, arm64-v8a and x86_64; x86-only emulators are unsupported.
Wear Compose 1.4.1 and TV Compose 1.0.0 are pinned to the existing Compose 1.7 baseline;
upgrading to Wear Material3 is a separate dependency upgrade.
