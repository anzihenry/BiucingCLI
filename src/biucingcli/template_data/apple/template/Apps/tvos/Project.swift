...
let appName = "{{SWIFT_MODULE_NAME}}_tvos"
...
let project = Project(
    ...
    targets: [
        .target(
            ...
            product: .app,
            bundleId: "{{BUNDLE_IDENTIFIER}}.tvos\(debugBundleSuffix)",
            deploymentTargets: .tvOS("{{MINIMUM_OS_VERSION}}"),
        ...
        .target(
            ...
            destinations: [.appleTv],
            product: .unitTests,
            bundleId: "{{BUNDLE_IDENTIFIER}}.tvos\(debugBundleSuffix).tests",
            deploymentTargets: .tvOS("{{MINIMUM_OS_VERSION}}"),
            infoPlist: .default,
            sources: ["Tests/**"],
