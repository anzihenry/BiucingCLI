...
- `internal/runtime`: bounded HTTP serving and graceful shutdown
...
- `SIGINT` and `SIGTERM` stop accepting new connections and drain in-flight HTTP requests.
- HTTP read, read-header, write, and idle timeouts are configured under `server` in `configs/config.yaml`.
- Graceful shutdown is bounded by `server.shutdown_timeout_seconds`; after the deadline, open connections are force-closed and the process exits non-zero.
