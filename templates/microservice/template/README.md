...
- `internal/runtime`: coordinated HTTP/gRPC serving and bounded shutdown
...
- `SIGINT` and `SIGTERM` start one coordinated shutdown for HTTP, gRPC, and OpenTelemetry.
- The gRPC health service switches to `NOT_SERVING` before draining active RPCs.
- HTTP stops accepting new work and drains in-flight requests with the configured deadline.
- If gRPC graceful stop exceeds `server.shutdown_timeout_seconds`, the server is force-stopped and the process exits non-zero.
- The runtime image checks `GET /healthz` every 10 seconds. Compose allows 15 seconds for the default 10-second shutdown window.
