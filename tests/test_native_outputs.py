...
class NativeOutputTests(CLIHelpers, unittest.TestCase):
    def test_create_android_renders_template(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn('include(":core:', settings_gradle)
            ...
            self.assertIn('implementation("com.example.demoandroid.components:network:" + componentVersions.getValue("network"))', app_build)
            self.assertIn('implementation("com.example.demoandroid.components:settings:" + componentVersions.getValue("settings"))', app_build)
            self.assertIn('testImplementation("com.example.demoandroid.components:testing:" + componentVersions.getValue("testing"))', app_build)
            ...
            self.assertIn("SettingsRoute(session.environment)", main_activity)
            self.assertIn("by viewModels()", main_activity)
            ...
            self.assertIn('title: String = "Demo Android"', home_route)
            self.assertIn("AppSectionCard(", home_route)
            self.assertIn("model.run()", home_route)
            ...
            self.assertIn("$(GRADLE) :$(APP_MODULE):bundleRelease", android_makefile)
            ...
            self.assertIn("$(GRADLE) :$(APP_MODULE):connectedDebugAndroidTest", android_makefile)
